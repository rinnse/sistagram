...
$(document).ready(function() {
	...
	$("#searchBtn").on("click", function() {
		...
		document.searchForm.action = "/admin/userMgt";
			    ...
			    <section style="margin-top:25px">
					...
					<section style="display: flex; align-items: center; margin-top: 30px;">
						...
						<form class="search_form" id="searchForm" name="searchForm" method="POST" >
						   ...
						   <select id="searchType" name="searchType" style="font-size: 1rem; width: 5rem; height: 2rem; margin-left:.5rem; ">
				               ...
				               <option value="2" <c:if test="${searchType == '2'}">selected</c:if>>작성자</option>
				               <option value="3" <c:if test="${searchType == '3'}">selected</c:if>>권한</option>
				            </select>
					...
					<!-- 사용자 리스트 -->
						<table class="table table-hover" style="border:1px solid #c4c2c2;">
				            ...
				            <tbody>
					<c:if test="${!empty umList}">
						<c:forEach items="${umList}" var="user" varStatus="status">
				            <tr class="table-thead-side">														<!-- 관리자가 회원관리할 수 있는 것은 권한 부여 -->
					...
					<c:if test="${empty umList}">
				         ...
				         <div class="paging-center" style="text-align: center;">
				            <!-- 페이징 시작 -->
							<c:if test="${!empty paging}">
							   ...
							   <c:forEach var="i" begin="${paging.startPage}" end="${paging.endPage}">
							   	   <c:choose>
				                        ...
				                        <a href="javascript:void(0)" style="background-color:black; border: none; font-size:12px;" class="btn2 btn-primary" onclick="fn_paging(${i})">${i}</a>
									   </c:when>
									   <c:otherwise>
				                        <h class="btn2 btn-primary" style="background-color:black; border: none;font-size:12px; font-weight:bold;">${i}</h>
							   		   </c:otherwise>
							       </c:choose>
							   </c:forEach>
				               <!-- 페이지 종료 -->
