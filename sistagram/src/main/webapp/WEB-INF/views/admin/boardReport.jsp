...
$(document).ready(function() {
	...
	$("#searchBtn").on("click", function() {
		...
		document.searchForm.action = "/admin/boardReport";
   ...
   if(confirm("해당 게시물을 삭제하시겠습니까?") == true) {
      $.ajax({
         ...
         success:function(res) {
             if(res.code == 0){
            	 alert("해당 게시물을 삭제되었습니다.");
            	 location.href = "/admin/boardReport";
             } else if(res.code == 400){
            	 alert("게시물 번호가 존재하지 않습니다.");
             } else if(res.code == 404){
            	 alert("게시물이 존재하지 않습니다.");
             } else if(res.code == 500){
            	 alert("게시물 처리 중 오류가 발생하였습니다.");
             } else {
            	 alert("오류가 발생하였습니다.");
             }
         },
		...
		<div class="aaa">
			<div>
			  <div>
				...
				<br />
			    <section>
					<b style="display: block; font-weight: bold;">게시물 신고 목록</b>
					...
					<c:if test="${!empty bmList}">
						<c:forEach items="${bmList}" var="board" varStatus="status">
				            <tr>
				                ...
				                <td>${board.regDate}</td>
				                <td>${board.status}</td>
				                <td><button type="button" class="deleteBtn" onclick="deletePost(${board.boardNum})">삭제하기</button></td>				    
				            </tr>
				    	</c:forEach>
